...
class AddonHelper
{
    ...
    public static function activateAddon(string $plugin)
    {
        ...
        if (check_ajax_referer('caseproof_growth_tools_install_addon', 'nonce')) {
            ...
            if (! current_user_can('activate_plugin')) {
                wp_send_json_error(
                    esc_html__('Could not activate addon. Please check user permissions.', 'cspf-growth-tools')
                );
            }
            ...
            if (isset($plugin)) {
                ...
                if (! is_wp_error($activate)) {
                    wp_send_json_success(esc_html__('Addon activated.', 'cspf-growth-tools'));
                }
            }

            wp_send_json_error(
                esc_html__('Could not activate addon. Please activate from the Plugins page.', 'cspf-growth-tools')
            );
        }

        wp_send_json_error(
            esc_html__('Could not activate addon. Please refresh page and try again.', 'cspf-growth-tools')
        );
    }
    ...
    public static function deactivateAddon(string $plugin)
    {
        ...
        try {
            ...
            wp_send_json_success(esc_html__('Plugin deactivated.', 'cspf-growth-tools'));
